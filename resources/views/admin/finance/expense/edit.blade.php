...
@section('admin')
    <div class="page-wrapper">
        <div class="page-content">
            ...
            <!--end breadcrumb-->
            <form action="{{ route('admin.finance.expense.update', ['id' => $expense->id]) }}" method="post"  >
                @method('put')
                @csrf
            <div class="card">
                ...
                <div class="form-body mt-4">
                    ...
                    <div class="border border-3 p-4 rounded">
                        <div class="mb-3">
                            ...
                            <input type="text" name="expense" class="form-control @error('expense') is-invalid  @enderror" value="{{old('expense',$expense->expense)}}" id="inputProductTitle" placeholder="Enter expense">
                            <span class="text-danger">{{$errors->first('expense')}}</span>
                        </div>
                        <div class="mb-3">
                            <label for="inputProductTitle" class="form-label">Date</label>
                            <input type="date" name="date" class="form-control @error('date') is-invalid  @enderror" value="{{old('date',$expense->date)}}" id="inputProductTitle" placeholder="Enter Date">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('Department') is-invalid @enderror" name="Department" id="Department" aria-label="Default select example">
                                ...
                                @foreach ($departments as $department)
                                    <option value="{{ $department->name }}" {{ old('Department',$expense->depart   ) == $department->name ? 'Selected' : NULL }}>
                                        {{ $department->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('descp') is-invalid  @enderror"  name="descp"  id="inputProductDescription" rows="3">{{ old('descp', $expense->descp) }}</textarea>
