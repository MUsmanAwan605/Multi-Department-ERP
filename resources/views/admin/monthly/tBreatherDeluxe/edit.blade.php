    ...
    <!--start page wrapper -->
		<div class="page-wrapper">
			<div class="page-content">
				...
				<!--end breadcrumb-->
			  <form method="post" action="/admin/monthly/tBreatherDeluxe/{{$tube->sr_no}}">
				@csrf
				{{method_field('put')}}
				<div class="card">
                           ...
                           <div class="border border-3 p-4 rounded">
							  ...
							  <div class="mb-3">
								...
								<select name="opr_name" id="opr_name" class="form-select @error('opr_name') is-invalid	@enderror">
									...
									<option value="extr_snl_lyr" {{old('opr_name',$tube->opr_name) == 'extr_snl_lyr' ? 'selected' : NULL }}>Extrusion(Single Layer)</option>
									<option value="blnk_ctng"{{old('opr_name',$tube->opr_name) == 'blnk_ctng' ? 'selected' : NULL }}>Blanck Cutting</option>
									<option value="vulcan"{{old('opr_name',$tube->opr_name) == 'vulcan' ? 'selected' : NULL }}>Vulcanization</option>
									<option value="washing"{{old('opr_name',$tube->opr_name) == 'washing' ? 'selected' : NULL }}>Washing</option>
									<option value="fnl_ctng"{{old('opr_name',$tube->opr_name) == 'fnl_ctng' ? 'selected' : NULL }}>Final Cutting</option>
