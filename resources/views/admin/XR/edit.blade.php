    ...
    <!--start page wrapper -->
		<div class="page-wrapper">
			<div class="page-content">
					...
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page">Edit X-R Record</li>
							</ol>
						</nav>
					</div>
					<div class="ms-auto">
				...
				<!--end breadcrumb-->
			  <form method="post" action="/admin/XR/{{$chart->id}}">
				@csrf
                {{method_field('put')}}
				<div class="card">
				  <div class="card-body p-4">
					<div class="d-flex justify-content-between">
                        ...
                        <div class="row">
                           ...
                           <div class="border border-3 p-4 rounded">
							  ...
							  <div class="mb-3">
								...
								<input type="text" class="form-control @error('lot_no') is-invalid @enderror" value="{{old('lot_no',$chart->lot_no)}}" name="lot_no" placeholder="Enter Lot Number">
								<span class="text-danger">{{ $errors->first('lot_no') }}</span>
							  </div>
							  <div class="mb-3">
								<label class="form-label" for="lot_size">Lot Size</label>
								<input type="text" class="form-control @error('lot_size') is-invalid @enderror" value="{{old('lot_size',$chart->lot_size)}}" name="lot_size" placeholder="Enter Lot Size">
								<span class="text-danger">{{ $errors->first('lot_size') }}</span>
							  </div>
                              <div class="mb-3">
								<label class="form-label" for="X1">Enter X1 Measurement</label>
								<input type="text" class="form-control @error('X1') is-invalid @enderror" value="{{old('X1',$chart->X1)}}" name="X1" placeholder="Enter X1 measurement">
								<span class="text-danger">{{ $errors->first('X1') }}</span>
							  </div>
							 <div class="mb-3">
								<label class="form-label" for="X2">Enter X2 Measurement</label>
								<input type="text" class="form-control @error('X2') is-invalid @enderror" value="{{old('X2',$chart->X2)}}" name="X2" placeholder="Enter X2 measurement">
								<span class="text-danger">{{ $errors->first('X2') }}</span>
							  </div>
							  <div class="mb-3">
								<label class="form-label" for="X3">Enter X3 Measurement</label>
								<input type="text" class="form-control @error('X3') is-invalid @enderror" value="{{old('X3',$chart->X3)}}" name="X3" placeholder="Enter X3 measurement">
								<span class="text-danger">{{ $errors->first('X3') }}</span>
							  </div>
                              <div class="mb-3">
								<label class="form-label" for="X4">Enter X4 Measurement</label>
								<input type="text" class="form-control @error('X4') is-invalid @enderror" value="{{old('X4',$chart->X4)}}" name="X4" placeholder="Enter X4 measurement">
								<span class="text-danger">{{ $errors->first('X4') }}</span>
							  </div>
                              <div class="mb-3">
								<label class="form-label" for="X5">Enter X5 Measurement</label>
								<input type="text" class="form-control @error('X5') is-invalid @enderror" value="{{old('X5',$chart->X5)}}" name="X5" placeholder="Enter X5 measurement">
							  ...
							  <div class="mb-3">
								...
								<select class="form-select @error('confirmation') is-invalid @enderror" name="confirmation" id="confirmation">
									...
									<option value="Approved" {{old('confirmation',$chart->confirmation) == 'Approved' ? 'selected' : NULL}}>Approved</option>
									<option value="Rejected" {{old('confirmation',$chart->confirmation) == 'Rejected' ? 'selected' : NULL}}>Rejected</option>
								</select>
					  ...
					  <div class="mb-3">
						...
						<textarea class="form-control @error('notes') is-invalid @enderror" value="{{old('notes',$chart->notes)}}" name="notes" id="notes" cols="30" rows="5">
						...
						<span class="text-danger">{{ $errors->first('notes') }}</span>
                              ...
                              <div class="mb-3">
								...
								<input type="text" class="form-control @error('sup_prcs_zone') is-invalid @enderror" value="{{old('sup_prcs_zone',$chart->sup_prcs_zone)}}" name="sup_prcs_zone" placeholder="Enter Supplier or Process Zone">
							  ...
							  <div class="mb-3">
								...
								<select class="form-select @error('su_chrctr') is-invalid @enderror" name="su_chrctr" id="su_chrctr">
									...
									<option value="Outer DIA" {{old('su_chrctr',$chart->su_chrctr) == 'Outer DIA' ? 'selected' : NULL}}>Outer DIA</option>
									<option value="Inner DIA" {{old('su_chrctr',$chart->su_chrctr) == 'Inner DIA' ? 'selected' : NULL}}>Inner DIA</option>
								</select>
								<span class="text-danger">{{ $errors->first('su_chrctr') }}</span>
							  </div>
                              <div class="mb-3">
								<label class="form-label" for="stndrd">Enter Standard</label>
								<input type="text" class="form-control @error('stndrd') is-invalid @enderror" value="{{old('stndrd',$chart->stndrd)}}" name="stndrd" placeholder="Enter Standard">
								<span class="text-danger">{{ $errors->first('stndrd') }}</span>
							  </div>
                              <div class="mb-3">
								<label class="form-label" for="smpl_frqncy">Enter Sampling Frequency</label>
								<input type="text" class="form-control @error('smpl_frqncy') is-invalid @enderror" value="{{old('smpl_frqncy',$chart->smpl_frqncy)}}" name="smpl_frqncy" placeholder="Enter Sampling Frequency">
								<span class="text-danger">{{ $errors->first('smpl_frqncy') }}</span>
							  </div>
                              <div class="mb-3">
								<label class="form-label" for="msrng_eqp">Enter Measuring Equipment</label>
								<input type="text" class="form-control @error('msrng_eqp') is-invalid @enderror" value="{{old('msrng_eqp',$chart->msrng_eqp)}}" name="msrng_eqp" placeholder="Enter Measuring Equipment">
