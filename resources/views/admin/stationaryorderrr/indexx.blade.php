                                ...
                                @endif --}}
                                 <td>
                                    <div class="d-flex order-actions">
                                        <a href="{{ route('adminstoreorder.edit',['id'=>$order->id]) }}" class=""><i class='bx bxs-edit'></i></a>
                                        <form method="POST" action="{{ route('adminstoreorder.destroy',['id'=>$order->id]) }}">
                                            @csrf
                                            {{ method_field('DELETE') }}
                                            <button type="submit" onclick="return confirm('Are you sure you want to delete?')" style="outline:none;border:none;background:transparent">
                                                <a href="{{ route('adminstoreorder.destroy',['id'=>$order->id]) }}" class="ms-3"><i class='bx bxs-trash'></i></a>
                                            </button>
                                        </form>
                                    </div>
                                </td>
