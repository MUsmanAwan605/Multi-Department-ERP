...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <!--end breadcrumb-->
        <form method="POST" action="{{ route('hr.loan.update',['id'=>$loan->id]) }}">
            {{ method_field('put') }}
            @csrf
            <div class="card">
                <div class="card-body p-4">
                    <h5 class="card-title">Add New Current Job</h5>
                    <hr/>
                      ...
                      <div class="row">
                         ...
                         <div class="border border-3 p-4 rounded">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error ('Date') is-invalid @enderror" value="{{ old('Date') }}" name="Date" id="inputProductTitle" value=" {{ old ('Date', $loan->date)}}">
                                <span class="text-danger text-uppercase ">{{ $errors->first('Date') }}</span>
                              </div>
                              <div class="mb-3">
                                <label for="So#" class="form-label">Serial Number</label>
                                <input type="text" class="form-control" name="Sno" id="inputProductTitle" value="{{  old('Sno',$loan->sno)  }}" readonly>
                              </div>
                              <div class="mb-3">
                                <label for="Department" class="form-label">Employee Name</label>

                                <input type="text" value="{{ old('Name',$loan->name) }}" class="form-control"  name="Name" id="bsValidation4" placeholder="Employee Name" readonly >
							...
							<div class="border border-3 p-4 rounded">
                              <div class="row g-3">
                                ...
                                <div class="mb-1">
                                    ...
                                    <input type="text" name="Department" id=""  class="form-control" value="{{ $loan->department }}" readonly>
                                  </div>

                            <div class="mb-1">
                                <label for="LoanAmount" class="form-label">Loan Amount</label>
                                <input type="number" class="form-control" name="LoanAmount" id="LoanAmount" value="{{ old('LoanAmount',$loan->remaining_balance )}}"  disabled>
                            </div>

                            <div class="mb-5">
                                <label for="LoanAmount" class="form-label">Loan repayment</label>
                                <input type="number" class="form-control @error ('loanpayment') is-invalid @enderror" name="loanpayment" id="inputProductTitle" value="{{ old('loanpayment')  }}" placeholder="Enter Loan Amount">
                                <span class="text-danger text-uppercase ">{{ $errors->first('loanpayment') }}</span>
                            </div>
