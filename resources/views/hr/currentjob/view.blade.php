                    ...
                    <table class="table mb-0" id="hiringTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                {{-- <td>{{ $record->id }}</td> --}}
                                <td style="vertical-align: middle">{{ $record->e_id }}</td>
                                <td style="vertical-align: middle">{{ $record->e_name }}</td>
                                <td style="vertical-align: middle">{{ $record->supervisor_name }}</td>
                               <td style="vertical-align: middle">{{ $record->department }}</td>
                               @foreach ($hirings as $hiring)
                               @if ($hiring->fname == $record->e_name)
                                   <td>
                           ...
                           @endforeach
                                ...
                                <td style="vertical-align: middle">
                                    <div class="d-flex order-actions">
                                        <a href="{{ route('hr.currentjob.edit',['id'=>$record->id]) }}"><i class='bx bxs-edit'></i></a>
                                        <form method="POST" action="{{ route('hr.currentjob.destroy',['id'=>$record->id]) }}">
                                            @csrf
                                            {{ method_field('DELETE') }}
                                            <button type="submit" onclick="return confirm('Are you sure you want to delete?')" style="outline:none;border:none;background:transparent">
                                                <a href="{{ route('hr.currentjob.destroy',['id'=>$record->id]) }}" class="ms-3"><i class='bx bxs-trash'></i></a>
                                            </button>
                                        </form>
                                    </div>
                                </td>
