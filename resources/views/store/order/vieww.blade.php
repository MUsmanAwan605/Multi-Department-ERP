            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-lg-flex align-items-center mb-4 gap-3">
                        <form method="GET"  action="{{ route('store.order.search') }}">
                            <div class="d-lg-flex">
                                <div class="col-8">
                                ...
                                <tbody>
                                    @foreach ($records as $record)
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle">{{ $record->qty }}</td>
                                            <td style="vertical-align: middle">{{ $record->dpt }}</td>
                                            <td style="vertical-align: middle; color: blue;">{{ $record->status }}</td>

                                            <td style="vertical-align: middle">
                                                <div class="d-flex order-actions">
                                                    <a href="{{ route('inventory.edit', $record->id) }}"
                                                        class=""><i class='bx bxs-edit'></i></a>
                                                    <form method="post"
                                                        action="{{ route('inventory.destroy', $record->id) }}">
                                                        @csrf
                                                        {{ method_field('DELETE') }}
